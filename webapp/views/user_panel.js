...
module.exports = Backbone.View.extend({
	...
	initialize: function() {
		// console.log("UUU", Kegbot.Router.args, Kegbot.User.get('username'))
		const username = Kegbot.Router.args[0];
		if (username === Kegbot.User.get('username')) {
			this.user = Kegbot.User;
		} else {
	...
	render: function() {
		this.scope = {
			...
			doors: this.doors,
			self: Kegbot.User,
			me: this.user.id === Kegbot.User.id,
		};
	...
	delUser: function() {
		if (confirm('Are you sure you want to delete '+
			this.user.get('username')+'?')) {
			this.user.destroy();
			Kegbot.Router.navigate('/admin', {trigger: true});
		}
	},
});
