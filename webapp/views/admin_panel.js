...
module.exports = Backbone.View.extend({
	...
	initialize: function() {
		if (!Kegbot.User.get('admin')) {
			// console.log("NOT ADMIN!!");
			return Kegbot.Router.navigate('', {trigger: true});
		}
	...
	render: function() {
		// console.log("RENDER MAIN:", Kegbot.User.get('admin'))
		this.scope = {
			doors: this.doors,
			users: this.users,
			settings: Kegbot.Settings,
			privateSettings: this.privateSettings,
			mailto: "mailto:?subject=Kegbot&body=Hey! you've been setup on the door. Visit " +
		...
		if (this.$(e.currentTarget).hasClass('toggle')) {
			...
			this.users.create({
				...
				success: _.bind(function(m) {
					...
					Kegbot.Router.navigate('/user/'+m.get('username'),
						{trigger: true});
				}, this),
				error: _.bind(function(m, resp) {
